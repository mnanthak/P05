...
public class CaboGame extends processing.core.PApplet {
  ...
  public void deckCheck() {
    ...
    System.out.println("Found correct numbers of action cards: "
        + (numActionPeek == 8 && numActionSpy == 8 && numActionSwitch == 8));
    ...
    System.out.println("Found correct numbers of each suit: "
        + (numHearts == 13 && numSpades == 13 && numDiamonds == 13 && numClubs == 13));

    // verify that the king of diamonds' getRank() returns -1
    for (int i = 0; i < deckCards.size(); i++) {
      if (deckCards.get(i).toString().equals("Diamonds 13") && deckCards.get(i).getRank() == -1) {
  ...
  public void handleCardSwap() {
    ...
    // find a card index from the current player's hand that the mouse is currently over
    int cardIndex = players[currentPlayer].getHand().indexOfMouseOver();

    // swap that card with the drawnCard
    BaseCard oldCard = players[currentPlayer].getHand().swap(drawnCard, cardIndex);
    ...
    for (Button button : buttons) {
      if (!button.getLabel().equals("End Turn")) {
        button.setActive(false);
      }
    }
    // erase all knowledge of the card at that index from the AI
  ...
  public void handleSpy() {
    ...
    // find a card index from any player's hand that the mouse is currently over
    int cardIndex = players[currentPlayer].getHand().indexOfMouseOver();
